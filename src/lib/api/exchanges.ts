...
class CryptoExchangeService {
  ...
  async getPrices(symbols: string[] = WATCHED_COINS): Promise<PriceData[]> {
    try {
      ...
      const [tickerPromises, marketDataPromise] = await Promise.all([
        Promise.all(
          symbols.map(symbol => 
            this.fetchWithTimeout(`${BINANCE_API}/ticker/24hr?symbol=${symbol}USDT`)
          )
        ),
      ...
      const allPrices = new Map(marketData.map((item: any) => [
  ...
  async getMarketMovers(): Promise<PriceData[]> {
    try {
      const response = await this.fetchWithTimeout(`${BINANCE_API}/ticker/24hr`);
      ...
      return data
        ...
        .sort((a: PriceData, b: PriceData) => Math.abs(b.change24h) - Math.abs(a.change24h))
        .slice(0, 25);
    } catch (error) {
  ...
  async executeTrade(order: OrderData): Promise<boolean> {
    try {
      ...
      if (exchange === 'binance' && apiKeys.binance) {
        ...
        const signature = await this.signRequest(params.toString(), apiKeys.binance.secretKey);
        ...
        const signature = await this.signRequest(message, apiKeys.coinbase.secretKey);
      ...
      throw new Error(`Exchange ${exchange} not supported or API keys not configured`);
    ...
    try {
      ...
      if (apiKeys.coinbase) {
        ...
        if (response.ok) {
          ...
          const nonZeroAccounts = accounts.filter((a: any) => parseFloat(a.balance) > 0);
